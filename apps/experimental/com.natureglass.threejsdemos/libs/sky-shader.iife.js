// Sky.js — IIFE conversion for switch-web-browser.
//
// Source: three-latest/examples/jsm/objects/Sky.js (Three.js r184).
//
// Transforms applied vs upstream:
//   1. ES module `import { ... } from 'three'` → pull from
//      `globalThis.__THREE_R184_STAGED__` at IIFE-execution time.
//   2. `export { Sky }` → `globalThis.Sky = Sky`.
//
// Embedded GLSL (vertex + fragment shaders) is byte-identical to
// upstream. Class body is byte-identical to upstream.
//
// Load order:
...
//   <script src="brewser://apps/ThreeJSDemos/libs/sky-shader.iife.js"></script>

(function () {

	const THREE = globalThis.__THREE_R184_STAGED__;
	if ( ! THREE ) {
		throw new Error( 'sky-shader.iife.js: __THREE_R184_STAGED__ missing — load three-latest.iife.js first' );
	}
